...
export class Html {
  ...
  addMenuEntry(item: any) {
    if (item.items && Array.isArray(item.items)) {
      ...
      const sub = createContainer(false, this.Base.options.width);
      sub.style.left =
  ...
  generateHtmlAndPublish(parent: any, item: any, submenu: any = undefined) {
  ...
  removeMenuEntry(index: any) {
    const element = find(`#${index}`, this.container.firstChild, false);
