(function(loader) {
    loader(function _commonDefine(turf) {
        ...
        Tin.prototype.updateTin2 = function() {
            var bbox = [];
            ...
            var bakTempTin = turf.tin(bakPoints, 'target');
            var centroid = turf.centroid(bakPoints);
            var centMerc = centroid.geometry.coordinates;
            var centIlls = transformArr(centroid, bakTempTin);
            var convex = turf.convex(bakPoints).geometry.coordinates[0];
            console.log(convex);
            var orthant = convex.reduce(function(prev, curr){
                var merc = curr;
                console.log(merc);
                var ills = transformArr(turf.point(curr), bakTempTin);
                var illsDelta = [ills[0] - centIlls[0], ills[1] - centIlls[1]];
                var mercDelta = [merc[0] - centMerc[0], centMerc[1] - merc[1]];
                console.log(merc + ' : ' + ills);

                if (mercDelta[0] == 0 || mercDelta[1] == 0) return prev;
                var idx = 0;
                if (mercDelta[0] > 0) idx += 1;
                if (mercDelta[1] > 0) idx += 2;
        ...
        function hit(point, tins) {
            ...
            for (var i=0; i< tins.features.length; i++) {
                ...
                console.log(point);
                console.log(tins.features[i].geometry.coordinates);
                if (inside) {
                    console.log("hit!!!!!!!!");
